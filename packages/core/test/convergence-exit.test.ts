...
describe("convergence exit", () => {
  ...
  it("does not apply entry-signal freshness to a held-position exit", async () => {
    ...
    expect(await exits(ctxWith([stale], [position()], 0.69))).toHaveLength(1);
  });

  it("sells once the market prices in the forecast", async () => {
    // Entered at 0.55, forecast 0.70, market now 0.69: 1pp left.
  ...
  it("holds while meaningful edge remains", async () => {
    // Market 0.60 against a 0.70 forecast: 10pp left, well outside 3pp.
    expect(await exits(ctxWith([sig()], [position()], 0.6))).toHaveLength(0);
  });

  it("sells a converged position at a loss", async () => {
    // The executable bid is below cost, but the forecast no longer favours it.
  ...
  it("exits on an overshoot past the forecast", async () => {
    // Market 0.78 above a 0.70 forecast: negative remaining edge.
  ...
  it("measures a NO position on its own token, not the YES mid", async () => {
    // NO forecast 0.70 (so YES 0.30). YES mid 0.29 → NO price 0.71, 1pp left.
    const got = await exits(ctxWith([sig({ side: "NO", prob: 0.7 })], [position({ side: "NO" })], 0.29));
    expect(got).toHaveLength(1);
    expect(got[0]!.reason).toMatch(/^converged: -1\.0pp edge left at mid 0\.710/);
  });
  ...
  it("holds through a signal-side flip while the held side keeps edge", async () => {
    // Held YES from 0.55, market fell to 0.10. The signal now sits on NO at
    // 0.75, i.e. Q still values YES at 0.25, 15pp above the price.
